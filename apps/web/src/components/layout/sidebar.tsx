...
export function Sidebar() {
  ...
  return (
    <aside className="w-64 h-screen fixed left-0 top-0 border-r border-[var(--color-border)] bg-white shadow-[var(--shadow-soft)] flex flex-col z-30 rounded-none">
      {/* Logo — clean light treatment with subtle gold wash */}
      <div className="h-16 flex items-center px-5 border-b border-[var(--color-border)] relative overflow-hidden bg-white">
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_30%_50%,color-mix(in_srgb,var(--color-accent)_6%,transparent),transparent_70%)]" />
        <div className="flex items-center gap-3 relative">
          <div className="relative">
            <Hexagon className="w-8 h-8 text-[var(--color-accent)] rotate-90" strokeWidth={1.2} />
            <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-[var(--color-foreground)]">
              YN
            </span>
          </div>
          <div>
      ...
      {/* Demo badge — gold-tinted banner */}
      {isDemo && (
        <div className="mx-3 mt-3 px-3 py-2.5 rounded-lg bg-[var(--color-accent)]/6 border border-[var(--color-accent)]/15 flex items-center gap-2.5">
            ...
            <Link
              ...
              className={cn(
                ...
                isActive && item.enabled && [
                  ...
                  // Navy indicator bar on active item
                  "before:absolute before:left-0 before:top-1/2 before:-translate-y-1/2 before:w-[3px] before:h-5 before:rounded-r-full before:bg-[var(--color-primary)]",
                ],
                !isActive && item.enabled && [
                  "text-[var(--color-muted)] hover:text-[var(--color-foreground)]",
                  "hover:bg-[var(--color-primary)]/3",
                ]
              )}
            >
              <item.icon
              ...
              {!item.enabled && (
                <span className="ml-auto text-[10px] bg-[var(--color-muted)]/10 px-2 py-0.5 rounded-full font-mono text-[var(--color-muted)]">
                  {t("nav.soon")}
                </span>
              )}
            </Link>
      ...
      <div className="px-3 py-2">
        <button
          ...
          className="w-full flex items-center gap-3 px-3 py-2 rounded-lg text-[13px] text-[var(--color-muted)] hover:text-[var(--color-foreground)] hover:bg-[var(--color-primary)]/3 transition-colors"
        >
          <Globe className="w-4 h-4" />
          <span>{t("lang.label")}</span>
          <span className="ml-auto text-[11px] bg-[var(--color-primary)]/6 px-2.5 py-0.5 rounded-full font-mono font-medium text-[var(--color-primary)]">
            {t("lang.switch")}
          </span>
        </button>
      </div>
