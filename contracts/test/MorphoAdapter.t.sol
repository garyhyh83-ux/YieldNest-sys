...
contract MorphoAdapterTest is Test {
    ...
    function setUp() public {
        ...
        vm.prank(vault);
        adapter = new MorphoAdapter(vault, address(usdc), address(morphoVault));
    ...
    function test_withdraw_insufficient_reverts() public {
        ...
        vm.expectRevert("MorphoAdapter: insufficient shares");
        adapter.withdraw(200 * 10 ** USDC_DECIMALS);
        vm.stopPrank();
    }

    function test_totalValue_increasesAfterDeposit() public {
        vm.startPrank(vault);
    ...
    function test_riskScore() public view {
        assertEq(adapter.getRiskScore(), 2500);
    }
    ...
    function test_getAPY_returnsZero() public view {
        // MetaMorpho APY is off-chain; on-chain returns 0
